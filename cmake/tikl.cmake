set(LOTTO_TIKL_VERSION "0.2")
set(LOTTO_TIKL_URL
    "https://github.com/db7/tikl/archive/refs/tags/v${LOTTO_TIKL_VERSION}.tar.gz"
)
set(LOTTO_TIKL_SHA256
    "SHA256=1920594387c7a40e9fe817ef5cac7737d1a5f0e95bc3b166e9bae056137631f9")

function(lotto_ensure_tikl OUT_TIKL OUT_TIKL_CHECK)
    if(DEFINED LOTTO_TIKL_EXECUTABLE AND DEFINED LOTTO_TIKL_CHECK_EXECUTABLE
       AND EXISTS "${LOTTO_TIKL_EXECUTABLE}" AND EXISTS "${LOTTO_TIKL_CHECK_EXECUTABLE}")
        ...
        if(_tikl)
            ...
            set(LOTTO_TIKL_EXECUTABLE "${_tikl}" CACHE INTERNAL "tikl executable")
            set(LOTTO_TIKL_CHECK_EXECUTABLE "${_tikl_check}" CACHE INTERNAL
                                                             "tikl-check executable")
            if(NOT TARGET tikl)
                add_custom_target(tikl)
            endif()
        else()
            set(_tikl_archive "${CMAKE_BINARY_DIR}/tikl-${LOTTO_TIKL_VERSION}.tar.gz")
            set(_tikl_source_dir "${CMAKE_BINARY_DIR}/tikl-${LOTTO_TIKL_VERSION}")
            ...
            if(NOT EXISTS "${_tikl_binary}")
                ...
                if(NOT _tikl_extract_rv EQUAL 0)
                    message(
                        FATAL_ERROR "Failed to extract tikl ${LOTTO_TIKL_VERSION}")
                endif()

                execute_process(
                    COMMAND
                        ${CMAKE_C_COMPILER} -std=c11 -D_POSIX_C_SOURCE=200809L -Wall
                        -Wextra -Wpedantic -Wshadow -Werror -O2 -o tikl tikl.c
                    WORKING_DIRECTORY "${_tikl_source_dir}"
                    RESULT_VARIABLE _tikl_build_rv)
                if(NOT _tikl_build_rv EQUAL 0)
                    message(
                        FATAL_ERROR "Failed to compile tikl ${LOTTO_TIKL_VERSION}")
                endif()
            endif()

            if(NOT EXISTS "${_tikl_binary}")
                message(
                    FATAL_ERROR "Expected tikl binary at ${_tikl_binary} was not created")
            endif()
            ...
            if(NOT _tikl_patch_rv EQUAL 0)
                message(
                    WARNING "Failed to patch tikl-check escaping logic; proceeding anyway")
            endif()

            set(LOTTO_TIKL_EXECUTABLE "${_tikl_binary}"
                CACHE INTERNAL "tikl executable")
            set(LOTTO_TIKL_CHECK_EXECUTABLE "${_tikl_check}"
    ...
    set(${OUT_TIKL} "${LOTTO_TIKL_EXECUTABLE}" PARENT_SCOPE)
    set(${OUT_TIKL_CHECK} "${LOTTO_TIKL_CHECK_EXECUTABLE}" PARENT_SCOPE)
endfunction()
