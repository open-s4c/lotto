...
;; record _anon_2
;; do not support anonymous struct or union
...
(define _topic
		(_enum '(EVENT_NONE = 0
			EVENT_ENGINE__START = 110
			EVENT_ENGINE__REPLAY_END = 111
			EVENT_ENGINE__BEFORE_CAPTURE = 112
			EVENT_ENGINE__BEFORE_MARSHAL_CONFIG = 113
			EVENT_ENGINE__BEFORE_MARSHAL_FINAL = 114
			EVENT_ENGINE__BEFORE_MARSHAL_PERSISTENT = 115
			EVENT_ENGINE__AFTER_UNMARSHAL_CONFIG = 116
			EVENT_ENGINE__AFTER_UNMARSHAL_FINAL = 117
			EVENT_ENGINE__AFTER_UNMARSHAL_PERSISTENT = 118
			EVENT_ENGINE__INFO_RECORD_LOAD = 119
			EVENT_ENGINE__INFO_RECORD_SAVE = 120
			EVENT_ENGINE__DELAYED_PATH = 121
			EVENT_ENGINE__NEXT_TASK = 122
			EVENT_DEADLOCK__DETECTED = 137
			EVENT_ENFORCE__VIOLATED = 138
			EVENT_TIMEOUT__TRIGGER = 139
			EVENT_RUNTIME__MEMMGR_INIT = 129
			EVENT_DRIVER__MEMMGR_INIT = 133
			EVENT_END_ = 140
	)))

(define _topic-domain
	'(EVENT_NONE
		EVENT_ENGINE__START
		EVENT_ENGINE__REPLAY_END
		EVENT_ENGINE__BEFORE_CAPTURE
		EVENT_ENGINE__BEFORE_MARSHAL_CONFIG
		EVENT_ENGINE__BEFORE_MARSHAL_FINAL
		EVENT_ENGINE__BEFORE_MARSHAL_PERSISTENT
		EVENT_ENGINE__AFTER_UNMARSHAL_CONFIG
		EVENT_ENGINE__AFTER_UNMARSHAL_FINAL
		EVENT_ENGINE__AFTER_UNMARSHAL_PERSISTENT
		EVENT_ENGINE__INFO_RECORD_LOAD
		EVENT_ENGINE__INFO_RECORD_SAVE
		EVENT_ENGINE__DELAYED_PATH
		EVENT_DEADLOCK__DETECTED
		EVENT_ENFORCE__VIOLATED
		EVENT_ENGINE__NEXT_TASK
		EVENT_TIMEOUT__TRIGGER
		EVENT_RUNTIME__MEMMGR_INIT
		EVENT_DRIVER__MEMMGR_INIT
		EVENT_END_
))

;; typedef _topic_t
(define _topic_t _topic)
...
;; record _anon_17
(define-cstruct _anon_17
	([__low _uint32]
	 [__high _uint32]
))
(define (alloc-anon_17)
	(cast (malloc 'raw (ctype-sizeof _anon_17))
	      _pointer
	      _anon_17-pointer))
...
;; record _logger_level
(define _logger_level
	(_enum '(LOGGER_ERROR = 0
		LOGGER_WARN = 1
		LOGGER_INFO = 2
		LOGGER_DEBUG = 3
)))

(define _logger_level-domain
	'(LOGGER_ERROR
		LOGGER_WARN
		LOGGER_INFO
		LOGGER_DEBUG
...
;; function _logger_set_level
(define _logger_set_level
	(_fun _logger_level -> _void))

;; function _logger_fatalf
(define _logger_fatalf
	(_fun _string -> _void))

;; function _logger_errorf
(define _logger_errorf
	(_fun _string -> _void))

;; function _logger_warnf
(define _logger_warnf
	(_fun _string -> _void))

;; function _logger_infof
(define _logger_infof
	(_fun _string -> _void))

;; function _logger_debugf
(define _logger_debugf
	(_fun _string -> _void))

;; function _logger_printf
...
(define _timer_delete
	(_fun _pointer -> _int32))

;; function _timer_getoverrun
(define _timer_getoverrun
	(_fun _pointer -> _int32))

;; record _timezone
(define-cstruct _timezone
	([tz_minuteswest _int32]
	 [tz_dsttime _int32]
))
(define (alloc-timezone)
	(cast (malloc 'raw (ctype-sizeof _timezone))
	      _pointer
	      _timezone-pointer))

;; record ___itimer_which
(define ___itimer_which
	(_enum '(ITIMER_REAL = 0
		ITIMER_VIRTUAL = 1
		ITIMER_PROF = 2
)))

(define ___itimer_which-domain
	'(ITIMER_REAL
		ITIMER_VIRTUAL
		ITIMER_PROF
))

;; typedef ___itimer_which_t
(define ___itimer_which_t _int32)

;; typedef _nanosec_t
(define _nanosec_t _uint64)
...
(define _replay_type-domain
	'(REPLAY_OFF
		REPLAY_ANY_TASK
		REPLAY_ON
))

;; typedef _replay_type_t
(define _replay_type_t _replay_type)

;; record _plan
(define-cstruct _plan
	([actions _uint32]
	 [next _task_id]
	 [with_slack _bool]
	 [reason _reason]
	 ...
	 [clk _clk_t]
	 [any_task_filter (_fun _task_id -> _bool)]
	 [args (_array/vector _arg 4)]
))
(define (alloc-plan)
	(cast (malloc 'raw (ctype-sizeof _plan))
	      _pointer
	      _plan-pointer))

;; typedef _plan_t
(define _plan_t _plan)
...
;; function _plan_done
(define _plan_done
	(_fun (_or-null _plan-pointer) -> _bool))

;; function _plan_print
(define _plan_print
	(_fun _plan -> _void))

;; function _plan_has
(define _plan_has
	(_fun _plan _action -> _bool))

;; function _action_str
(define _action_str
	(_fun _action -> _string))
...
;; function _engine_fini
(define _engine_fini
	(_fun (_or-null _context-pointer) _reason -> _int32))

;; function _engine_capture
(define _engine_capture
	(_fun (_or-null _context-pointer) -> _plan))

;; function _engine_resume
(define _engine_resume
	(_fun (_or-null _context-pointer) -> _void))

;; function _engine_return
(define _engine_return
	(_fun (_or-null _context-pointer) -> _void))

;; function _sequencer_fini
(define _sequencer_fini
	(_fun (_or-null _context-pointer) _reason -> _void))

;; function _sequencer_capture
(define _sequencer_capture
	(_fun (_or-null _context-pointer) -> _plan))

;; function _sequencer_resume
(define _sequencer_resume
	(_fun (_or-null _context-pointer) -> _void))

;; function _sequencer_return
(define _sequencer_return
	(_fun (_or-null _context-pointer) -> _void))

;; function _sequencer_get_clk
...
;; pending:   _clock_getcpuclockid
;; missing -> _pid_t


;; pending:   _clock_getres
;; missing -> _timespec


;; pending:   _clock_gettime
;; missing -> _timespec


;; pending:   _clock_nanosleep
;; missing -> _timespec


;; pending:   _clock_settime
;; missing -> _timespec
...
;; record _sigevent
;; incomplete definition
(define _sigevent _void)(define _sigevent-pointer _pointer)

;; function _timer_create
(define _timer_create
	(_fun _clockid_t (_or-null _sigevent-pointer) (_cpointer _timer_t)  -> _int32))
...
;; pending:   _stream_close
;; missing -> _stream_s


;; pending:   _stream_read
;; missing -> _stream_s


;; pending:   _stream_reset
;; missing -> _stream_s
...
;; record _stream_s
;; incomplete definition
(define _stream_s _void)(define _stream_s-pointer _pointer)
...
;; pending:   _sys_clock_gettime
;; missing -> _timespec


;; pending:   _sys_gettimeofday
;; missing -> _timeval


;; pending:   _sys_nanosleep
;; missing -> _timespec
...
;; pending:   _timer_gettime
;; missing -> _itimerspec


;; pending:   _timer_settime
;; missing -> _itimerspec
...
;; pending:   _value
;; missing -> _value::union (anonymous at /home/diogo/Workspaces/lotto/src/include/lotto/base/value.h:19:5)
