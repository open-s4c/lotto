...
{
    bool ok = true;
    ok &= driver_try_dump_files(dir, (driver_file_t[]){
                                         {.path    = "debug.gdb",
                                          .content = debug_gdb,
                                          .len     = debug_gdb_len},
                                         {NULL},
    ...
    ok &= driver_try_dump_files(
        python_dir,
        (driver_file_t[]){
            {.path = "command.py", .content = command_py, .len = command_py_len},
            {.path = "decorator.py",
            ...
            {.path = "matcher.py", .content = matcher_py, .len = matcher_py_len},
            {.path = "handler.py", .content = handler_py, .len = handler_py_len},
            {NULL},
        });
    return ok;
}
