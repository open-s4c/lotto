...
void emit_udf_exit(unsigned int cpu_index, void *udata);
...
static inline void
bind_udf_exit_callback(capture_point *cp, struct qemu_plugin_insn *insn)
{
    qemu_plugin_register_vcpu_insn_exec_cb(insn, emit_udf_exit,
...
/* Legacy compatibility while old frontend code still exists in-tree. */
static inline void
register_exit_cb(capture_point *cp, struct qemu_plugin_insn *insn)
{
    bind_udf_exit_callback(cp, insn);
}
