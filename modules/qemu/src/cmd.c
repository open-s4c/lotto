...
{
    const char *legacy_direct = sys_getenv("LOTTO_QEMU_LEGACY_DIRECT");
    if (legacy_direct == NULL || legacy_direct[0] == '\0') {
        char **reexec_argv =
            sys_calloc((size_t)(args->argc + 3), sizeof(char *));
        int r            = 0;
    ...
    int nplugins = 0;
    char runtime_modules[MAX_LIST_STR];
    runtime_modules[0] = '\0';

    if (plugins_enabled) {
        plugins[nplugins++] = _qemu_plugin_spec(
            _select_qemu_plugin(args, default_module_dir, fallback_module_dir,
                                verbose_enabled),
            flags);
        _build_runtime_plugin_modules(
            runtime_modules, sizeof(runtime_modules),
            _select_qemu_plugin(args, default_module_dir, fallback_module_dir,
                                verbose_enabled),
            default_module_dir, fallback_module_dir, verbose_enabled);
    }
    const char *default_qemu_args[] = {
        "-machine",   "virt", "-m", mem,          "-cpu", "cortex-a57",
        "-nographic", "-smp", cpu,  "-no-reboot", NULL,
    ...
    int res = _execvpe_with_filtered_env(argv, runtime_modules);
    if (res != 0) {
...
ON_DRIVER_REGISTER_COMMANDS({
    ...
    subcmd_register(qemu, "qemu", "[<options>] [--] [qemu_args...]",
                    "Shortcut for 'stress -Q -- [qemu_args...]' (set "
                    "LOTTO_QEMU_LEGACY_DIRECT=1 for legacy direct mode)",
