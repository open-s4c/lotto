...
static void
bind_udf_exit_reason(struct qemu_plugin_insn *insn, uint32_t reason)
{
    qlotto_exit_event_t *exit_ev = sys_malloc(sizeof(*exit_ev));
    capture_point *exit_cp       = sys_malloc(sizeof(*exit_cp));
    exit_ev->reason              = reason;
    *exit_cp                     = (capture_point){.chain_id = INTERCEPT_EVENT,
                                                   .type_id  = EVENT_QLOTTO_EXIT,
                                                   .func     = __FUNCTION__,
                                                   .payload  = exit_ev};
    bind_udf_exit_callback(exit_cp, insn);
}
                     ...
                     const qemu_translation_policy_t *policy)
{
    ...
    if (opcode == LOTTO_EXIT_SUCCESS_VAL || opcode == LOTTO_EXIT_STOP_VAL ||
        opcode == LOTTO_QEMUQUIT_A64_VAL) {
        bind_udf_exit_reason(insn, REASON_SUCCESS);
        return;
    }

    if (opcode == LOTTO_EXIT_FAIL_VAL) {
        bind_udf_exit_reason(insn, REASON_ASSERT_FAIL);
    }
}
