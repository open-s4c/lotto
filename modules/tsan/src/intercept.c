...
#define sized_arg(s, uv)                                                       \
    ({                                                                         \
        ...
        switch (s) {                                                           \
            ...
            case 2:                                                            \
                _arg =                                                         \
                    (arg_t){.value.u16 = uv.u16, .width = ARG_U16};            \
                break;                                                         \
            case 4:                                                            \
                _arg =                                                         \
                    (arg_t){.value.u32 = uv.u32, .width = ARG_U32};            \
                break;                                                         \
            case 8:                                                            \
                _arg =                                                         \
                    (arg_t){.value.u64 = uv.u64, .width = ARG_U64};            \
                break;                                                         \
            default:                                                           \
                ASSERT(0);                                                     \
                break;                                                         \
...
PS_SUBSCRIBE(CAPTURE_BEFORE, EVENT_MA_RMW, {
    ...
    context_t *c = ctx(.cat  = CAT_BEFORE_RMW,
                       .func = ev->func,
...
PS_SUBSCRIBE(CAPTURE_BEFORE, EVENT_MA_XCHG, {
    ...
    context_t *c = ctx(.cat  = CAT_BEFORE_XCHG,
                       .func = ev->func,
                       .args = {arg_ptr(ev->addr), arg(size_t, ev->size), sized_arg(ev->size, ev->val)});
    intercept_capture(c);
    return PS_OK;
...
PS_SUBSCRIBE(CAPTURE_BEFORE, EVENT_MA_FENCE, {
    ...
    context_t *c = ctx(.cat  = CAT_BEFORE_FENCE,
                       .func = ev->func,);
    intercept_capture(c);
    return PS_OK;
})
...
PS_SUBSCRIBE(CAPTURE_AFTER, EVENT_MA_FENCE, {
    ...
    context_t *c = ctx(.cat  = CAT_AFTER_FENCE,
                       .func = ev->func,);
    intercept_capture(c);
    return PS_OK;
})
