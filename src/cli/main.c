...
#define LOTTO_BOOTSTRAP_OLD_DICE_DSO "LOTTO_DRIVER_BOOTSTRAP_OLD_DICE_DSO"
...
main(int argc, char **argv)
{
    ...
    if (getenv(LOTTO_BOOTSTRAP_ENV) == NULL) {
        ...
        if (getenv("DICE_DSO") != NULL && getenv("DICE_DSO")[0] != '\0') {
            setenv(LOTTO_BOOTSTRAP_OLD_DICE_DSO, getenv("DICE_DSO"), true);
        } else {
            setenv(LOTTO_BOOTSTRAP_OLD_DICE_DSO, "", true);
        }
        setenv("LD_PRELOAD", preload.buf, true);
        setenv("DICE_DSO", "liblotto-driver.so", true);
    ...
    {
        const char *value = getenv(LOTTO_BOOTSTRAP_OLD_DICE_DSO);
        if (value != NULL) {
            if (value[0] == '\0') {
                unsetenv("DICE_DSO");
            } else {
                setenv("DICE_DSO", value, true);
            }
            unsetenv(LOTTO_BOOTSTRAP_OLD_DICE_DSO);
        }
    }
