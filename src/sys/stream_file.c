...
stream_file_write(stream_t *stream, const char *buf, size_t size)
{
    ...
    return write(stream_file->fd, buf, size);
}
...
{
    stream_file_t *stream_file = (stream_file_t *)stream;
    return read(stream_file->fd, buf, size);
}
...
{
    stream_file_t *stream_file = (stream_file_t *)stream;
    lseek(stream_file->fd, 0, SEEK_SET);
}
...
stream_file_in(stream_t *s, const char *fname)
{
    ...
    if (fd == -1) {
        dprintf(STDERR_FILENO, "stream_file_in: open('%s') failed: %s\n", fname,
                strerror(errno));
...
stream_file_out(stream_t *s, const char *fname)
{
    ...
    if (fd == -1) {
        dprintf(STDERR_FILENO, "stream_file_out: open('%s') failed: %s\n", fname,
                strerror(errno));
        logger_fatalf("error: could not open file '%s'\n", fname);
    }
